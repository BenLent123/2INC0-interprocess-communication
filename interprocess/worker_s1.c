...
int main (int argc, char * argv[])
{
   ...
   S1_queue_T21 req;
    ...
    if(req_channel == (mqd_t)-1){
        ...
        exit(EXIT_FAILURE);
    }else{perror("worker 1 - request channel opening succeeded");}
    ...
    while((1)){
		...
		if(mq_sent_w==0) mq_recieved_w = mq_receive(req_channel, (char*)&req, sizeof(S1_queue_T21),0);
        if(mq_recieved_w == -1){
        ...
        if(req.request_id == -1){
            ...
            else {fprintf(stderr,"worker 1 sent work\n");}
        }
